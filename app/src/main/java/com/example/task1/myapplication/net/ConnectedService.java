package com.example.task1.myapplication.net;
...
import static com.example.task1.myapplication.net.ConnectConstants.*;
...
public class ConnectedService {
     ...
     * 同意求和
     */
    public void agreeEqual(){
        byte[] data = new byte[2];
        data[0] = 3;
        data[1] = ConnectConstants.EQUAL_AGREE;
        Message msg = new Message();
        msg.obj = data;
        mSender.sendMessage(msg);
    }
     ...
     * 拒绝求和
     */
    public void rejectEqual(){
        byte[] data = new byte[2];
        data[0] = 3;
        data[1] = ConnectConstants.EQUAL_REJECT;
        Message msg = new Message();
        msg.obj = data;
        mSender.sendMessage(msg);
    }
    public void equal(){
        byte[] data = new byte[2];
        data[0] = 3;
        data[1] = ConnectConstants.EQUAL_ASK;
        Message msg = new Message();
        msg.obj = data;
        mSender.sendMessage(msg);
    }
    ...
    private void processNetData(byte[] data){
        ...
        switch (type) {
            ...
            case ROLLBACK_REJECT:
                ...
                break;
            case EQUAL_ASK:
                mRequestHandler.sendEmptyMessage(EQUAL_ASK);
                break;
            case EQUAL_AGREE:
                mRequestHandler.sendEmptyMessage(EQUAL_AGREE);
                break;
            case EQUAL_REJECT:
                mRequestHandler.sendEmptyMessage(EQUAL_REJECT);
                break;
            default:
                break;
        }
